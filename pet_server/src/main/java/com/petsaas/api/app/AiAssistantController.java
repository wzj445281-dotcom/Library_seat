...
public class AiAssistantController {
    ...
    // 请在 application.yml 中配�?your_google_api_key
    ...
    public ApiResponse<String> chat(@RequestBody Map<String, String> params) {
        ...
        if (userQuestion.contains("�?) || userQuestion.contains("推荐") || userQuestion.contains("吃什�?) || userQuestion.contains("用品")) {
            // 查询销量最高的3个商�?
        ...
        String systemPrompt = String.format(
                "你是一位专业的宠物医生助理，名字叫'宠爱小助�?。你的职责是�?.根据用户的描述判断宠物的健康状况�?.推荐本店的宠物商品（如狗粮、驱虫药）�?.引导用户预约线下的洗澡或美容服务。请用温柔、关怀的语气回答。\n\n" +
                "当前用户�?s，当前会员积分：%d。\n\n" +
                "%s" +
